...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    socket.on('new_order', (data) => {
      ...
      let notificationMessage = `Order #${data.order.order_id} from ${data.order.city}`;
      let notificationType = 'order';
      let notificationTitle = 'New Order Received';
      
      if (data.inventoryAlert && data.inventoryAlert.hasLowStock) {
        notificationType = 'inventory_alert';
        notificationTitle = '⚠️ Inventory Alert';
        notificationMessage = `${data.inventoryAlert.message} Order #${data.order.order_id} from ${data.order.city}`;
      }
      ...
      const newNotification = {
        ...
        read: false,
        data: data.order,
        inventoryAlert: data.inventoryAlert
      };
      ...
      try {
        ...
        if (newOrders.length > 0) {
          ...
          setNotifications(prev => {
            ...
            const existingIds = new Set(prev.map(n => n.id));
            const uniqueNewNotifications = newNotifications.filter(n => !existingIds.has(n.id));
            setUnreadCount(count => count + uniqueNewNotifications.length);
            return [...uniqueNewNotifications, ...prev];
          });
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell size={20} />
        {unreadCount > 0 && (
          <span className={`absolute -top-1 -right-1 w-5 h-5 text-white text-xs rounded-full flex items-center justify-center font-bold animate-pulse ${
            notifications.some(n => !n.read && n.type === 'inventory_alert') ? 'bg-red-500' : 'bg-blue-500'
          }`}>
              ...
              notifications.map(notification => (
                <div
                  ...
                  className={`p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${
                    !notification.read ? 
                      notification.type === 'inventory_alert' ? 'bg-red-50 border-l-4 border-l-red-500' : 
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-lg ${
                      ...
                      notification.type === 'inventory_alert' ? 'bg-red-100 text-red-600' :
                    ...
                    >
                        ...
                        <h4 className={`font-medium text-sm ${
                          ...
                          {!notification.read && (
                            <span className={`ml-2 inline-block w-2 h-2 rounded-full ${
                              notification.type === 'inventory_alert' ? 'bg-red-500' : 'bg-blue-500'
                            }`}></span>
                          )}
                        </h4>
                      ...
                      {notification.inventoryAlert && (
                        <div className="mt-2 p-2 bg-red-50 rounded border border-red-200">
                          <p className="text-xs font-medium text-red-800 mb-1">Low Stock Items:</p>
                          {notification.inventoryAlert.lowStockItems.map((item, index) => (
                            <div key={index} className="text-xs text-red-600 mb-1">
                              • {item.name}: {item.currentStock} → {item.newStock} (Priority: {item.restockPriority})
                            </div>
                          ))}
                        </div>
                      )}
