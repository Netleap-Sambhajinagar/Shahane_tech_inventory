...
exports.createOrder = async (req, res, next) => {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
        ...
        return res.status(400).json({ 
            ...
            details: 'Items are required and must be an array'
        });
    }
    ...
    try {
        ...
        // Store order items
        const lowStockItems = []; // Track items with insufficient stock
        if (items && Array.isArray(items)) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                console.log(`✅ Found product: ${product.product_id} - ${product.name}`);
                ...
                if (stockStatus === 'insufficient') {
                    console.log(`⚠️ Low stock warning for ${name}. Available: ${currentStock}, Requested: ${orderQuantity}`);
                    lowStockItems.push({
                ...
                // Immediately update inventory for this product
                const newStock = currentStock - orderQuantity;
                const quantitySold = product.quantity_sold || 0;
                const newQuantitySold = quantitySold + orderQuantity;
                
                // Calculate restock priority based on new stock level
                let newRestockPriority = product.restock_priority || 0;
                if (newStock <= 0) {
                    newRestockPriority = 1; // Highest priority - out of stock/negative
                } else if (newStock <= 10) {
                ...
                console.log(`📊 Updating inventory for ${actualProductId}: Stock: ${currentStock} → ${newStock}, Sold: ${quantitySold} → ${newQuantitySold}, Priority: ${newRestockPriority}`);
                
                // Update product inventory immediately
                ...
                console.log(`✅ Product updated: ${updateResult.affectedRows} rows affected`);
            }
        }
        ...
        if (io) {
            const notificationData = {
                ...
                inventoryAlert: lowStockItems.length > 0 ? {
                    hasLowStock: true,
                    lowStockItems: lowStockItems,
                    message: `${lowStockItems.length} item(s) have insufficient stock and require immediate restocking!`
                } : null,
            ...
            if (lowStockItems.length > 0) {
                console.log(`⚠️ INVENTORY ALERT: ${lowStockItems.length} items have insufficient stock`);
                lowStockItems.forEach(item => {
                    console.log(`   - ${item.name}: ${item.currentStock} → ${item.newStock} (Priority: ${item.restockPriority})`);
                });
            }
        }
